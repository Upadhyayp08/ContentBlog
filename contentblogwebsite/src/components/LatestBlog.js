...
function LatestBlog() {
  ...
  // Dispatch and selector are used to call a function and get data from the function
  useEffect(() => {
    dispatch(usedata.getAllBlogData());
  }, []);

  const data = useSelector((store) => store.appReducer);
  // to set the data on page refresh
  useEffect(() => {
    setBlogData(data.payload);
  }, [data]);
  // Splitting the array in chunks to show it in the carousel
  ...
  // Calling the passing by passing the required arguments
